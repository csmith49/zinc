...
(* further updated - no debruijn anymore, clashes with contexts *)
type 'a t =
  | Abs of Variable.t * 'a t
  | App of 'a t * 'a t
  | Symbol of 'a
type 'a term = 'a t
...
module Zipper = struct
  ...
  let rec to_term (z : 'a t) : 'a term =
    let current = fst z in match snd z with
      | [] -> fst z
      | (A x) :: bs -> to_term (Abs (x, current), bs)
